...
def connect_to_Marian():
    ...
    while error:
        try:
            ...
            printLOG(0,"Error: waiting for Marian server to start. Retrying in 2 seconds.","")
            time.sleep(2)
            error=True
    return(ws)
...
def translate_segment_part(segment,ucfirst=True):
    print("TRANSLATESEGMENTPART",segment)
    try:
...
###sentencepiece
if sentencepiece:
    sp= spm.SentencePieceProcessor(model_file=spmodelSL, out_type=str, add_bos=bos_annotate, add_eos=eos_annotate)
    sp2= spm.SentencePieceProcessor(model_file=spmodelSL, out_type=str)
    spTL= spm.SentencePieceProcessor(model_file=spmodelTL, out_type=str, add_bos=bos_annotate, add_eos=eos_annotate)
