...
def translate_segment_part(segment):
    try:
        ...
        if len(tagrestorer.remove_tags(segment).strip())==0:
            return(segment)
        ...
        if hastags and MTUOCServer_restore_tags and MTUOCServer_restore_strategy=="mtengine":
            selectedtranslationTags=tagrestorer.restore_tags(segmentWordsTok, segmentTAGSTok, wordali, selectedtranslationWordsTok)
        ...
        #if not truecaser==None:        
        #    selectedtranslation=selectedtranslation[0].upper()+selectedtranslation[1:]
        if is_first_letter_upper(segment):
            selectedtranslation=upper_case_first_letter(selectedtranslation)
    ...
    if escape_html_output:
        segment=html.escape(selectedtranslation)
    if unescape_html_output:
        segment=html.unescape(selectedtranslation)
        
    if not change_output_files[0]=="None":
        for change in change_output:
            try:
                tofind=change[0]
                tochange=change[1]
                if segment.find(tofind)>-1:
                    regexp="\\b"+tochange+"\\b"
                    selectedtranslation=re.sub(regexp, tochange, selectedtranslation)
            except:
                print("ERROR:",sys.exc_info())
