...
class TagRestorer():
    ...
    def restore_tags(self,SOURCENOTAGSTOK, SOURCETAGSTOK, SELECTEDALIGNMENT, TARGETNOTAGSTOK):
        taglist=["<tag0>","<tag1>","<tag2>","<tag3>","<tag4>","<tag5>","<tag6>","<tag7>","<tag8>","<tag9>","<tag10>","</tag0>","</tag1>","</tag2>","</tag3>","</tag4>","</tag5>","</tag6>","</tag7>","</tag8>","</tag9>","</tag10>"]
        ...
        try:
            ...
            for token in SOURCETAGSTOKLIST:
                if token=="▁" and SOURCETAGSTOKLIST[pos+1] in taglist:
                    SOURCETAGSTOKLIST.pop(pos)
                pos+=1
        except:
            pass
        ...
        cont=0
        acumulat=0
        print("*****TARGETTAGLIST",TARGETTAGLIST)
        for token in SOURCETAGSTOKLIST:
            if self.isSTag(token) or self.isSClosingTag(token):
                if not token in ["<s>","</s>"]:
                    if self.isSTag(token):
                        tagsposition[token]=cont
                        TARGETTAGLIST.insert(ali[cont],token)
                    elif self.isSClosingTag(token):
                        tagsposition[token]=cont
                        TARGETTAGLIST.insert(ali[cont-1]+1+acumulat,token)
                    acumulat+=1
            else:
                cont+=1
        print("*****tagsposition",tagsposition)
        print("*****TARGETTAGLIST",TARGETTAGLIST)    
